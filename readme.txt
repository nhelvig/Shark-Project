                               CS 61B  Project 1
                                Sharks and Fish
                     Due 5:30pm Friday, February 22, 2013
...
3shark is born in Ocean 1, and is fed during the transition from Ocean 2 to
...
Each time nextRun() is invoked, it returns a different run--represented as an
array of length two--until every run has been returned.  (The array is
constructed in nextRun(), and can be discarded by the calling method after use.
The two-int array returned should not be part of your RunLengthEncoding data
structure!  It must be freshly constructed for the sole purpose of returning
two ints.)  The array encodes the run by storing the cell contents at index
zero--either FISH, SHARK, or EMPTY--and the run length at index one.  The first
time nextRun() is invoked, it returns the first run in the encoding, which
contains cell (0, 0).  After every run has been returned, nextRun() returns
null, which lets the calling program know that there are no more runs in the
encoding.
...
You cannot change any of the prototypes in RunLengthEncoding.java.  Again, we
will test your code by calling your methods directly.
...
autograding program Test.class before you submit it.  Change (cd) to your pj1
directory, which should contain Ocean.java, RunLengthEncoding.java, and any
other .java files you wish to submit.  If your implementation uses .java files
in addition to those we have specified, have no fear:  the "submit" program
will ask you which .java files in your pj1 directory you want to submit.  From
your pj1 directory, type "submit pj1".
...
submit will be graded, unless you inform your reader that you would prefer to
have an earlier submission graded instead.
